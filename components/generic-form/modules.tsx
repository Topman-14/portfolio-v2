...
export function AsyncSelectField({ field, form }: FieldComponentProps) {
  ...
  useEffect(() => {
    if (field.fetchOptions) {
      if (Array.isArray(field.fetchOptions)) {
        ...
        setOptions(filtered);
      } else if (typeof field.fetchOptions === 'function' && debouncedSearchTerm) {
        setLoading(true);
        field.fetchOptions(debouncedSearchTerm)
          .then(setOptions)
          .catch((error) => {
  ...
  const selectedOption = options.find(option => option.value === form.watch(field.name));
...
export function SelectField({ field, form }: FieldComponentProps) {
  const [open, setOpen] = useState(false);
  const selectedOption = field.options?.find(option => option.value === form.watch(field.name));
...
export function renderField(field: FieldConfig, form: FieldComponentProps['form']) {
  const commonProps = {
